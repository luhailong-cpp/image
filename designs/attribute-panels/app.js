(function () {
  ...
  function dialog(title, body, action) {
    ...
    $('#dialogAction')?.addEventListener('click',() => { dialogAction(); $('#infoDialog').close(); });
  }
  function renderIdentity() {
    if(mode === 'hero') {
      const e=M.get('hero').entity;
      $('#identity').innerHTML=`<div class="identity-name"><h2>${e.name}</h2><p><span class="role-tag">道童</span> ${e.level}级 · ${e.subtitle}</p></div><div class="portrait-stage"><img class="hero-art" src="assets/${e.image}" alt="金色发带、玉绿背心的Q版道童清玄"></div><div class="identity-bottom"><div class="progress-label"><span>修行经验</span><span>68%</span></div><div class="exp-track" role="progressbar" aria-label="修行经验" aria-valuenow="68" aria-valuemin="0" aria-valuemax="100"><i></i></div><div class="identity-caption">心随道行 · 万象归一</div></div>`;
    } else {
      $('#identity').innerHTML=`<div class="pet-list-heading"><h2>我的宝宝</h2><span>携带 <b>4</b> / 8</span></div><div class="pet-list" aria-label="选择宝宝">${M.entities.filter(e=>e.kind==='pet').map(e=>`<button type="button" class="pet-card ${e.id===selectedPet?'active':''}" data-pet="${e.id}" aria-pressed="${e.id===selectedPet}"><div class="pet-avatar"><img src="assets/${e.image}" alt=""><small>${e.level}</small></div><div><strong>${e.name}</strong><p>${e.subtitle} · ${e.element}</p></div><span class="pet-status">${e.id===battlePet?'参<br>战':e.id===selectedPet?'已<br>选':'休<br>息'}</span></button>`).join('')}</div><div class="pet-list-foot"><span>与灵同行，山海不远</span><span>4 只灵宠</span></div>`;
    }
  }
  function renderExtra(e) {
    if(mode==='hero') {
      ...
      $('#battle').addEventListener('click',()=>{battlePet=battlePet===e.id?null:e.id;renderIdentity();renderExtra(e);toast(battlePet?`${e.name}已出战`:`${e.name}已休息`);});
  ...
  function renderNumbers() {
    ...
    $('#stats').innerHTML=M.statKeys.map(key=>`<div class="stat"><div class="stat-label">${statLabels[key]}</div><span class="stat-value">${(s.stats[key]-s.delta[key]).toLocaleString('en-US')}</span>${s.delta[key]?`<span class="stat-delta">+${s.delta[key]}</span>`:''}</div>`).join('');
  ...
  function render() {
    ...
    if(mode==='pet') $('#stats').insertAdjacentHTML('beforebegin',`<div class="pet-detail-head"><img class="pet-detail-art" src="assets/${e.image}" alt="${e.subtitle}${e.name}"><div class="pet-detail-title"><h3>${e.name}</h3><span class="pet-chip">${e.subtitle}</span><p>${e.level}级 · ${e.element}系灵宠</p></div></div>`);
    renderExtra(e);
    $('#scheme').value=schemes[e.id];
    $('#footerText').textContent=mode==='hero'?'一念入道，五行随心':'山海相伴，灵契同心';
    $('#allocations').innerHTML=M.attributes.map((label,i)=>`<div class="attribute-row"><div class="attribute-label"><label for="range${i}" title="${attrNotes[i]}">${label}</label><span id="attributeValue${i}" class="attribute-value"></span></div><button type="button" class="stepper" data-minus="${i}" aria-label="减少${label}">−</button><div class="slider-box"><input type="range" id="range${i}" step="1" data-range="${i}" aria-label="分配${label}属性点"></div><button type="button" class="stepper" data-plus="${i}" aria-label="增加${label}">+</button></div>`).join('');
    renderNumbers();
  }
  ...
  document.addEventListener('click',event=>{
    ...
    if(target.dataset.pet){selectedPet=target.dataset.pet;render();}
