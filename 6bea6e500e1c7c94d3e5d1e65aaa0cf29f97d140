...
对已经含透明通道的输入，装配器按技能测出的裁框、尺寸和位置重放 RGBA 合成，使用不带第二次 alpha 蒙版的粘贴，避免源 alpha 自乘。所有缩放仍为等比。

原始方图边长不要求恰好被 4 整除；按技能整数格子切分，记录外侧 0–3 px 余数，并确认排除区域没有可见主体。任何可见主体落在余数区域则失败。
