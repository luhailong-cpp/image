...
const manifest={product:copy.product.title,version:'native-q5.2',date:'2026-09-06',asset_count:outputRecords.length,original_dimensions_preserved:true,source_builder:rel(fileURLToPath(import.meta.url)),source_vectors:['native_q5/base.svg','native_q5/controls.svg','native_q5/labels.svg'],layer_contract:{base:'Transparent decorative frame/plaque + approved Q hero; no labels.',controls:'Transparent independently drawn native control skins, badge icons, and demo state markers; no labels.',combined:'Exact alpha-over of matching base and controls; no labels.',root_rgba:'Compatibility alias of the 5120 transparent combined UI.',root_rgb:'Approved main-city scene + light veil + 2560 combined UI + separate labels SVG from existing copy JSON. Visual reference only.'},runtime_note:'This rebuild does not connect a game client. Use individual controls/state assets and render text from real data. Combined PNGs illustrate one default arrangement and do not implement input or selection.',sources:[{path:rel(heroPath),sha256:sha(await fs.readFile(heroPath)),role:'Existing 1024px true-alpha Q hero, resampled for corner decoration.'},{path:rel(cityPath),sha256:sha(await fs.readFile(cityPath)),role:'Existing main-city background; root RGB visuals only.'},{path:'qdao_ui_redesign_v5/copy.zh-CN.json',sha256:sha(await fs.readFile(path.join(repo,'qdao_ui_redesign_v5/copy.zh-CN.json'))),role:'Existing approved product name and demo data.'}],hero_placement_2560:heroRect,control_placements_2560:placements,text_placements_2560:textNodes,outputs:outputRecords};
