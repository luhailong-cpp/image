...
IDS=('00_reference_topright_boy','02_fire_talisman_boy')
def sha(p):
...
current={p.relative_to(PROJECT).as_posix():p for p in (PROJECT/'Assets/Resources/World/Characters').rglob('*') if p.is_file() and not p.relative_to(PROJECT).as_posix().startswith(FAMILY+'/')}
