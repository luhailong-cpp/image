...
## 最新目标与状态（2026-09-17）
**实际进度：七套完整Q版布局参考已出齐（各1254×1254）；4K局部样图1张；完整64K主城0套。** 新增的[全部主城Q版参考](builtin_q64_all_city_references/README.md)已经覆盖当前客户端四城七外观，但不能计为1792张正式4K图块。各套[离线分块坐标计划](q64_production_plans/README.md)已建立；本轮全部使用内置入口，按GPT Image2.0和最高可用画质要求生成，未调用独立API。
...
用户最新决定：独立 API 需要额外收费，因此本轮改用**内置 GPT Image 2.0 路线（`builtin_image_gen`）**。严格指定 GPT Image 2.5 的要求及 API 凭证阻塞已撤销；**本轮不调用单独计费 API，不需要配置 Key**。活跃制作目录为 [builtin_q64_r10_c07](E:/work/image/qdao_city_tiles_4k_20260916/builtin_q64_r10_c07/)，已生成并拼好天墉城 `r10_c07` 的[首块4096² Q版样图](builtin_q64_r10_c07/output/tianyong_r10_c07_q64_4k_candidate.png)，由16张原生1254²细节图拼接，未放大最终美术；[样图说明与预览](builtin_q64_r10_c07/README.md)。局部总览、原像素细节与九个交点已检查，尚未完成整城、相邻正式图块或实机验收；当前1张样图、0张正式验收交付块。七套外观共计划 1792 块，计划数量不代表已生成。

内置工具没有模型、尺寸或质量选择器，后端由宿主管理。本轮统一Q版参考及16张原生图的 C2PA 生成动作均标注 `softwareAgent={name:gpt-image,version:2.0}`；本轮17张均保存实际来源与元数据，工具仍没有型号选择器。逐张保留实际尺寸、来源和可获得的元数据，未核实的型号不作确认声明。核验限于元数据解析，未做密码学验签，见[本轮17张来源核验](E:/work/image/qdao_city_tiles_4k_20260916/builtin_q64_r10_c07/qa/builtin-model-audit.json)。原 [GPT Image 2.5 API 执行包](E:/work/image/qdao_city_tiles_4k_20260916/q64_gpt25/执行包.md)保留为停用历史方案，不再等待或配置该方案的 API 凭证。
