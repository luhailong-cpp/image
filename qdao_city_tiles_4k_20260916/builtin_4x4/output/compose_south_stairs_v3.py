...
DEST = OUT / "tianyong_plaza_4k_candidate_v3.png"
REPORT = OUT / "south_stairs_v3_assembly.json"
HELPER = ROOT.parents[1] / "tianyong_festival_hd_20260910" / "seam_helpers.py"
BAND = 96
OVERLAP = 230
UNION = [909, 2842, 3187, 4096]
...
def prepare_inputs():
    ...
    if plan["union"] != UNION or plan["patches"] != expected_patches or plan["overlap"] != OVERLAP:
...
def seam_fn():
    spec = importlib.util.spec_from_file_location("city_v3_minimum_seam",HELPER)
...
def main():
    ...
    if DEST.exists() or REPORT.exists():
        raise ValueError("v3 already exists; use --check rather than overwriting its audit record")
    minimum = seam_fn()
    ...
    save(result,DEST)
    qa=[]
    joined=QA/"v3_fullspan_native_join_2278x1254.png"
    save(repair,joined)
    qa.append({"file":str(joined),"kind":"native_join_no_resize","size":[2278,1254],"sha256":sha(joined)})
    overview_path=QA/"v3_overview_1024.png"
    ...
    crops={
        "v3_south_stairs_left_seam_100pct":[459,3196,1359,4096],
        "v3_south_stairs_right_seam_100pct":[2737,3196,3637,4096],
        "v3_south_stairs_internal_seam_100pct":[1598,3196,2498,4096],
        "v3_south_stairs_top_seam_100pct":[1598,2392,2498,3292],
        "v3_south_stairs_fullwidth_100pct":[650,2842,3450,4096],
    }
    ...
    full_preview=QA/"v3_south_stairs_fullwidth_overview_1400.png"
    ...
    report={
        ...
        "plan":{"file":str(REPAIR/"plan.json"),"sha256":sha(REPAIR/"plan.json"),"union":UNION},
        ...
        "composition":{"method":"native left/right minimum-error hard seam, then four hard border seams into original candidate",
                       "internalOverlapPixels":OVERLAP,"joinedNativeSize":[2278,1254],"borderSearchBandPixels":BAND,
                       ...
                       "outsideUnionPixelsPreserved":True,"outputPixelsAreUnmodifiedSourceSamples":True,
        ...
        "qa":qa,
        "visualQa":{"status":"pending","concern":"Full-width regeneration moves outer seams off stair treads, but internal join, railing boundaries and all four edges still require visual review."}
    }
