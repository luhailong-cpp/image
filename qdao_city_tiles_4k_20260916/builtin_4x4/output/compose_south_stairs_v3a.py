...
DEST = OUT / "tianyong_plaza_4k_candidate_v3a.png"
REPORT = OUT / "south_stairs_v3a_assembly.json"
HELPER = ROOT.parents[1] / "tianyong_festival_hd_20260910" / "seam_helpers.py"
BAND = 96
OVERLAP = 230
SOURCE_UNION = [909, 2842, 3187, 4096]
UNION = [909, 3260, 3187, 4096]
SOURCE_CROP = [0, 418, 2278, 1254]
...
def seam_fn():
    spec = importlib.util.spec_from_file_location("city_v3a_minimum_seam",HELPER)
...
def main():
    ...
    if DEST.exists() or REPORT.exists():
        raise ValueError("v3a already exists; use --check rather than overwriting its audit record")
    minimum = seam_fn()
    ...
    repair = repair[418:1254].copy()
    if repair.shape != (836,2278,3):
        raise AssertionError("Cropped native repair must remain 2278x836 without resampling")
    base = rgb(BASE)
    x0,y0,x1,y1 = UNION
    ...
    edge_top = minimum(np.transpose(old[:BAND],(1,0,2)),np.transpose(repair[:BAND],(1,0,2)))
    ...
    save(result,DEST)
    qa=[]
    joined=QA/"v3a_cropped_native_join_2278x836.png"
    save(repair,joined)
    qa.append({"file":str(joined),"kind":"native_join_no_resize","size":[2278,836],"sourceCrop":SOURCE_CROP,"sha256":sha(joined)})
    overview_path=QA/"v3a_overview_1024.png"
    ...
    crops={
        "v3a_south_stairs_left_seam_100pct":[459,3196,1359,4096],
        "v3a_south_stairs_right_seam_100pct":[2737,3196,3637,4096],
        "v3a_south_stairs_internal_seam_100pct":[1598,3196,2498,4096],
        "v3a_south_stairs_top_seam_100pct":[1598,2810,2498,3710],
        "v3a_south_stairs_fullwidth_100pct":[650,2842,3450,4096],
    }
    ...
    full_preview=QA/"v3a_south_stairs_fullwidth_overview_1400.png"
    ...
    report={
        ...
        "composition":{"method":"native left/right minimum-error hard seam, then four hard border seams into original candidate",
                       "internalOverlapPixels":OVERLAP,"joinedNativeSize":[2278,1254],"joinedSourceCrop":SOURCE_CROP,"croppedNativeSize":[2278,836],"actualCompositeRect":UNION,"borderSearchBandPixels":BAND,
                       ...
                       "outsideUnionPixelsPreserved":True,"allPixelsAboveY3260MatchOriginal":bool(np.array_equal(result[:3260],base[:3260])),"outputPixelsAreUnmodifiedSourceSamples":True,
        ...
        "qa":qa,
        "visualQa":{"status":"pending","concern":"Restricted crop preserves original pixels above y3260; inspect the new top join, all stair treads and outside railing edges before acceptance."}
    }
