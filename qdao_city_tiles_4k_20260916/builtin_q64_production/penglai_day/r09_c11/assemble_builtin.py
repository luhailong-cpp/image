#!/usr/bin/env python3
"""Mechanical Q64 penglai_day r09_c11 4x4 native-patch assembly. No generation or source resizing."""
...
OUTPUT = ROOT / "output"
QA = ROOT / "qa"
ART = OUTPUT / "penglai_day_r09_c11_4k_candidate.png"
...
def load_sources() -> tuple[list[list[np.ndarray]], list[dict]]:
    ...
    for row in range(1, GRID + 1):
        ...
        for column in range(1, GRID + 1):
            ...
            if tile_id == "r02_c02":
                repair = ROOT / "repairs" / "r02_c02_regeneration"
                native = repair / "native" / f"{tile_id}.png"
                record_path = repair / "native" / f"{tile_id}.record.json"
                prompt = repair / "prompts" / f"{tile_id}.prompt.txt"
                guide = repair / "guides" / f"{tile_id}.png"
...
def assemble() -> dict:
    ...
    manifest = {
        ...
        "published": False,
        "scope": "One Q/chibi penglai_day r09_c11 4096-square candidate at65536 city density,18.75x18.75world units,16 native1254 patches. External neighboring delivery tiles are unverified; not a complete city.",
        "backendModelVerified": False,
        "backendModel": None,
        ...
        "extendedContext": {"file": "output/extended-context.png", "pixels": [4326,4326], "sha256": sha256(OUTPUT / "extended-context.png"), "outerContextPerSide": 115},
