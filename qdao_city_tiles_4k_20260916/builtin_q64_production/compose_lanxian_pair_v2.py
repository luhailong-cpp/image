...
for v in ('lanxian_day','lanxian_spring'):
 d=P/v/'pair_r08_c06_c07';out=d/'output';qa=d/'qa';fields=d/'registration-v2';fields.mkdir(exist_ok=True)
 ...
 def paste(native,rect,edges,label,sources):
  ...
  Image.fromarray(result).save(qa/f'{label}-after_100pct.png')
 curve=P/v/'r08_c07/repairs/curved_paving';nat,src=source(curve/'native/curve.record.json')
 paste(nat,[6736,390,7990,1644],('left','right','top','bottom'),'curve',[src])
 ...
 if v=='lanxian_spring':
  ...
  for i,n in enumerate(arrays[1:],2):
   ...
   strip=tmp.transpose(1,0,2);metrics.append(metric)
 ...
 art=Image.fromarray(a);pair=out/'pair-v2.png';art.save(pair)
 ext=Image.open(out/'pair-extended-v1.png').convert('RGB');ext.paste(art,(115,115));ext.save(out/'pair-extended-v2.png')
 candidates=[]
 for i,c in enumerate((6,7)):
  tile=f'r08_c{c:02d}';f=out/f'{v}_{tile}_q64_4k_candidate_pair_v2.png';crop=art.crop((i*4096,0,(i+1)*4096,4096));crop.save(f)
  assert np.array_equal(np.array(crop),a[:,i*4096:(i+1)*4096])
  ex=ext.crop((i*4096,0,i*4096+4326,4326));ex.save(out/f'{tile}-extended-context-v2.png')
 ...
 art.resize((1600,800),Image.Resampling.LANCZOS).save(qa/'pair-v2-overview.jpg',quality=80)
 for i,y in enumerate((0,800,1600,2400,3196)):art.crop((3646,y,4546,y+900)).save(qa/f'boundary-v2-{i}_100pct.png')
 ...
 (out/'pair-v2-assembly.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2))
 print(v,sha(pair))
