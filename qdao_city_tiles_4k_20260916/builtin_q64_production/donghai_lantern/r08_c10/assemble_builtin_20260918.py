#!/usr/bin/env python3
"""Mechanical Q64 Donghai Lantern Festival r08_c10 4x4 native-patch assembly. No generation or source resizing."""
...
OUTPUT = ROOT / "output"
QA = ROOT / "qa"
ART = OUTPUT / "donghai_lantern_r08_c10_q64_4k_candidate.png"
...
def load_sources() -> tuple[list[list[np.ndarray]], list[dict]]:
    ...
    for row in range(1, GRID + 1):
        ...
        for column in range(1, GRID + 1):
            ...
            submitted = Path(record['submittedReferencePath'])
            if not submitted.is_file() or sha256(submitted) != record['submittedReferenceSha256']:
                raise ValueError(f'Submitted primary reference hash mismatch: {tile_id}')
            actual_refs = record.get('actualInputReferences')
            if actual_refs:
                if record.get('inputCount') != len(actual_refs):
                    raise ValueError(f'Actual input count mismatch: {tile_id}')
                for idx, ref in enumerate(actual_refs, 1):
                    ref_path = Path(ref['path'])
                    if ref.get('imageIndex') != idx or not ref_path.is_file() or sha256(ref_path) != ref['sha256']:
                        raise ValueError(f'Actual input provenance mismatch: {tile_id}/{idx}')
                selected = int(record['selectedImageIndex'])
                if Path(actual_refs[selected-1]['path']).resolve() != submitted.resolve():
                    raise ValueError(f'Selected geometry reference mismatch: {tile_id}')
            for ref in record.get('additionalSubmittedReferences', []):
                if not Path(ref['path']).is_file() or sha256(Path(ref['path'])) != ref['sha256']:
                    raise ValueError(f'Additional submitted reference hash mismatch: {tile_id}')
...
def write_qa(image: Image.Image) -> list[dict]:
    ...
    crops = {
        "market_upper_left_100pct": (350, 300, 1250, 1200),
        "paving_lower_left_100pct": (500, 2800, 1400, 3700),
        "paving_lower_right_100pct": (2800, 2800, 3700, 3700),
    }
...
def validate_saved(manifest: dict, entries: list[dict]) -> dict:
    ...
    with Image.open(ART) as candidate:
        ...
        extended = manifest["extendedContext"]
        extended_path = ROOT / extended["file"]
        if sha256(extended_path) != extended["sha256"]:
            raise ValueError("Extended context hash differs from assembly")
        with Image.open(extended_path) as extended_image:
            extended_image.load()
            if extended_image.size != (ASSEMBLED, ASSEMBLED):
                raise ValueError("Extended context dimensions differ")
            core = extended_image.crop((HALO, HALO, HALO+FINAL, HALO+FINAL))
            if not np.array_equal(np.asarray(core), np.asarray(candidate)):
                raise ValueError("Candidate differs from exact extended-context core crop")
    ...
    return {"passed": True, "nativeSources": len(entries), "outputSize": [FINAL, FINAL],
            "qaFiles": len(manifest["qa"]), "status": manifest["status"]}
...
def assemble() -> dict:
    ...
    prep = plan.get("guidePreparation", {})
    ready_from_final_day = (prep.get("method", "").startswith("Exact native crops from final selected day geometry") and Path(prep.get("daySource", "")).is_file() and sha256(Path(prep["daySource"])) == prep.get("daySourceSha256"))
    if not ready_from_final_day and prep.get("status") not in {"ready_unified_style_reference", "ready_unified_local_reference_with_exact_native_left_overlap"}:
    ...
    manifest = {
        ...
        "published": False,
        "scope": "One more-Q/chibi Donghai Lantern Festival r08_c10 4096-square candidate at 65536-square whole-city density, covering 18.75x18.75 world units, from 16 native1254 patches; not a complete city map.",
        "backendModelVerified": False,
        "backendModel": None,
        "userSelectedModel": "GPT Image 2.5 requested; host-managed/unverified",
        ...
        "extendedContext": {"file": "output/extended-context.png", "width": ASSEMBLED, "height": ASSEMBLED, "sha256": sha256(OUTPUT / "extended-context.png")},
        "crossDeliveryTileSeamsVerified": False,
        ...
        "qa": qa_items,
        "visualQa": {"status": "pending", "note": "Numeric seam differences are diagnostic only; Q-style consistency, market stall/basket/paving geometry and all seam intersections require visual review."},
