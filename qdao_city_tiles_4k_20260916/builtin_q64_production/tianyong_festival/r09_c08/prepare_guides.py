...
import numpy as np
p=Path(r'E:/work/image/qdao_city_tiles_4k_20260916/builtin_q64_production/tianyong_festival/r09_c08');src=Path(r'C:\Users\luyua\.codex\generated_images\01a0ad4d-500d-7c83-87fa-a0a2b62155fe\exec-4d7c33f3-024d-4c3d-a7c1-afd762bd0fa7.png');dst=p/'style-reference.png';assert not dst.exists();assert Image.open(src).size==(1254,1254);shutil.copyfile(src,dst)
sha=lambda f:hashlib.sha256(f.read_bytes()).hexdigest()
refs=[p/n for n in ('layout-input.jpg','left-style-input.jpg','bottom-style-input.jpg')];prompt=p/'style-reference.prompt.txt'
(p/'style-reference.record.json').write_text(json.dumps({'route':'builtin_image_gen','role':'unified layout/style reference only, excluded from detail counts','backendModelVerified':False,'actualNativePixels':[1254,1254],'sourceOutputPath':str(src),'sourceOutputSha256':sha(src),'outputPath':str(dst),'outputSha256':sha(dst),'promptSha256':sha(prompt),'submittedImages':[{'path':str(f),'sha256':sha(f)} for f in refs],'toolCall':{'name':'image_gen.imagegen','num_last_images_to_include':3},'createdAtUtc':datetime.now(timezone.utc).isoformat()},indent=2),encoding='utf-8')
...
plan=json.loads((p/'plan.json').read_text());plan['styleReferenceSha256']=sha(dst);plan['status']='reference_ready_native_detail_production_in_progress';plan['guidePreparation']={'canvasPixels':[4326,4326],'method':'Reference-only resize and exact230px left/bottom neighbor constraints; each1254patch is then generated natively','guidePixelsNotDelivery':True,'neighborConstraints':layout['neighborConstraints']}
patches=[]
for row in range(4):
 for col in range(4):
...
print('16 reference guides ready;24 identical-overlap checks passed')
