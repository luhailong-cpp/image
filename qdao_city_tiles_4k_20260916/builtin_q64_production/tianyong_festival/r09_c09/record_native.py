...
plan=json.loads((P/'plan.json').read_text());patch=next(e for e in plan['patches'] if e['id']==ident.split('.')[0])
...
refs=[Path(f) for f in patch['submittedImages']];assert sha(refs[0])==patch['guideSha256']
if len(refs)>1: assert sha(refs[1])==plan['styleReferenceSha256']
...
rec['id']=patch['id'];rec['versionStem']=ident
