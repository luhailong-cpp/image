...
source=p/'output_v2/vertical-extended-context.png';canvas=np.array(Image.open(source).convert('RGB'));before=canvas.copy();r=p/'repairs/v3'
placements={'short_joint':{'box':(1421,0,2675,1254),'roi':(340,40,930,590),'edges':('left','right','top','bottom')}}
...
quadsource=p/'output_v2/row10-extended-context.png';quad=np.array(Image.open(quadsource).convert('RGB'));quad[:,:4326]=canvas[4096:];qe=Image.fromarray(quad);qe.save(out/'row10-extended-context.png');row=qe.crop((115,115,16499,4211));row.save(out/'row10_16384x4096_candidate.png')
files=[]
