#!/usr/bin/env python3
"""Mechanical Q64 penglai_mid_autumn r09_c10 4x4 native-patch assembly. No generation or source resizing."""
...
QA = ROOT / "qa" / "v2"
ART = OUTPUT / "penglai_mid_autumn_r09_c10_4k_candidate_v2.png"
...
def load_sources() -> tuple[list[list[np.ndarray]], list[dict]]:
    ...
    for row in range(1, GRID + 1):
        ...
        for column in range(1, GRID + 1):
            ...
            if tile_id == "r04_c02":
                repair = ROOT / "repairs" / "r04_c02_warm_lighting"
                native = repair / "repair-native.png"
                record_path = repair / "repair.record.json"
                prompt = repair / "repair.prompt.txt"
...
def write_qa(image: Image.Image) -> list[dict]:
    ...
    crops = {
        "roof_ridge_100pct": (2200, 800, 3100, 1700),
        "facade_corner_100pct": (1300, 2800, 2200, 3700),
        "parasol_stall_100pct": (2800, 2800, 3700, 3700),
    }
...
def validate_saved(manifest: dict, entries: list[dict]) -> dict:
    ...
    extended_path = ROOT / manifest["extendedContext"]["file"]
    if sha256(extended_path) != manifest["extendedContext"]["sha256"]:
        raise ValueError("Extended context hash differs")
    with Image.open(extended_path) as extended, Image.open(ART) as cropped:
        if extended.size != (4326,4326) or not np.array_equal(np.asarray(extended.crop((115,115,4211,4211))),np.asarray(cropped)):
            raise ValueError("Exact 115px outer crop contract failed")
...
def assemble() -> dict:
    ...
    manifest = {
        ...
        "published": False,
        "scope": "One Q/chibi penglai_mid_autumn r09_c10 4096-square candidate at65536 city density,18.75x18.75world units,16 native1254 patches. External neighboring delivery tiles are unverified; not a complete city.",
        "backendModelVerified": False,
        "backendModel": None,
        ...
        "extendedContext": {"file": relative(OUTPUT / "extended-context.png"), "pixels": [4326,4326], "sha256": sha256(OUTPUT / "extended-context.png"), "outerContextPerSide": 115},
        "externalDeliveryTileSeams": "not_yet_generated_or_accepted",
        "nativeSources": entries,
        "repair": {"count": 1, "recordFile": "repairs/r04_c02_warm_lighting/repair.record.json",
                   "baseNativeCount": 16, "baseNativePreserved": True,
                   "selection": "15 original native patches plus one native1254 replacement for r04_c02",
                   "baselineAssemblyFile": "output/assembly.json",
                   "baselineAssemblySha256": sha256(ROOT / "output" / "assembly.json")},
        "seamMetrics": metrics,
        "output": {"file": relative(ART), "width": FINAL, "height": FINAL, "mode": "RGB",
                   "format": "PNG", "sha256": sha256(ART)},
        "qa": qa_items,
        "visualQa": {"status": "pending", "note": "Numeric seam differences are diagnostic only; Q-style consistency, roof tile lanes, facade, parasol ribs and all seam intersections require visual review."},
