...
im=Image.open(D/'output_v3/extended-context.png').convert('RGB')
rows=[]
for repair in j(D/'output_v3/assembly.json')['repairs']:
...
for name,box in [('upper-outer-context-return',(7780,15,8834,330)),('bottom-outer-context-return',(7780,4000,8834,4311))]:
    f=Q/f'{name}.png';assert not f.exists();im.crop(box).save(f)
...
report={'source':str(D/'output_v3/extended-context.png'),'sourceSha256':sha(D/'output_v3/extended-context.png'),'actualRoiEdges':rows,'identicalV2Qa':identical,'changedV2Qa':changed}
