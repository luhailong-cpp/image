#!/usr/bin/env python3
"""Single-reference provenance variant; Mechanical Q64 lanxian_spring r08_c08 4x4 native-patch assembly. No generation or source resizing."""
...
OUTPUT = ROOT / "output"
QA = ROOT / "qa"
ART = OUTPUT / "lanxian_spring_r08_c08_q64_4k_candidate.png"
...
def load_sources() -> tuple[list[list[np.ndarray]], list[dict]]:
    ...
    for row in range(1, GRID + 1):
        ...
        for column in range(1, GRID + 1):
            ...
            selection=read_json(ROOT/"source-selection.json").get(tile_id,{}) if (ROOT/"source-selection.json").exists() else {}
            stem=selection.get("nativeStem",tile_id)
            native = ROOT / "native" / f"{stem}.png"
            record_path = ROOT / "native" / f"{stem}.record.json"
            pre_record = read_json(record_path)
...
def assemble() -> dict:
    ...
    manifest = {
        ...
        "published": False,
        "scope": "One more-Q/chibi lanxian_spring r08_c08 4096-square candidate at 65536-square whole-city density, covering 18.75x18.75 world units, from 16 native1254 patches; not a complete city map.",
        "backendModelVerified": False,
        "backendModel": None,
        "userSelectedModel": "ChatGPT Images 2.5 target; host managed; actual variant unverified",
