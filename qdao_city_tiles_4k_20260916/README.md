...
当前已按用户要求转用内置 `image_gen`，正在制作**天墉城中央广场的首块 4096 × 4096 候选图**。这是局部样块制作，四个地点的七套地图外观尚未全部完成，客户端地图尚未发布或替换。

## 当前实际制作路线

当前布局为 4 × 4 个生成分区。已落盘的原生输出为 1254 × 1254，每张包含 1024 × 1024 核心和四边各 115 像素上下文，相邻分区重叠 230 像素；完成后用于拼成首块 4096 × 4096 候选。**这是多张原生图片的拼接，不是单次原生 4K 生成。** 实际完成情况以文件和逐张记录为准，不以预设数量表示成功。
