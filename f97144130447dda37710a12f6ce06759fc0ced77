...
for direction in ['S','W','E','N','SW','NW','NE','SE']:
 path=char/'source'/(direction+'.png')
...
for kind,dirs in rp.ROWS.items():
 ...
 final.save(char/'source'/(kind+'_assembled.png'))
 prompt='This is a deterministic 4x4 layout assembled from four built-in ImageGen 2x2 directional walk sheets. Original generation prompts: '+', '.join('prompts/walk-'+d+'.txt' for d in dirs)+'. Row order '+','.join(dirs)+'. Layout provenance in source/assembly.json. No synthetic, copied, mirrored, rotated or deformed poses.'
 (char/'prompts'/(kind+'_assembled.txt')).write_text(prompt,encoding='utf8')
(char/'source'/'assembly.json').write_text(json.dumps(record,ensure_ascii=False,indent=2),encoding='utf8')
