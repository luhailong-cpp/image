...
def main():
 ...
 baseline=json.loads((ROOT/'qdao_gpt_image2_refresh_v7/inventory.json').read_text(encoding='utf8'));old={x['path']:x for x in baseline['assets']};checks=[]
 for f in meta['frames']:
  ...
  assert sha(src)==rec['sha256'] and sha(src)!=old[key]['sha256']
 ...
 if not a.check:
  atlas.save(png,optimize=True);meta['art_version']='daoist-q-gpt-image2-v7';meta['source_paths_relative_to']='this JSON directory';meta['atlas_sha256']=sha(png);JSON.write_text(json.dumps(meta,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
  for group in ['west_eight_immortals_redrawn_100_600x600','character_artifacts_redrawn_24_600x600']:
   ...
   manifest.write_text(json.dumps({'count':len(items),'size':[600,600],'redrawn':True,'art_version':'daoist-q-gpt-image2-v7','items':items},ensure_ascii=False,indent=2)+'\n',encoding='utf8')
  (PACK/'manifest_redrawn_600x600.json').write_text(json.dumps({'count':124,'size':[600,600],'redrawn':True,'art_version':'daoist-q-gpt-image2-v7','groups':['west_eight_immortals_redrawn_100_600x600','character_artifacts_redrawn_24_600x600'],'generation_records':rel(HERE/'records',PACK),'source_sheets_policy':'All v7 generated source sheets, exact prompts and processing records are retained in qdao_gpt_image2_refresh_v7/icons; final files retain original paths.','atlas':'fairygui_atlas/qstyle_fairygui_atlas_600.png'},ensure_ascii=False,indent=2)+'\n',encoding='utf8')
  (HERE/'manifest.json').write_text(json.dumps({'art_version':'daoist-q-gpt-image2-v7','count':124,'baseline_commit':baseline['baseline_commit'],'image_generator':'built-in image_gen / GPT Image 2','skill':'generate2dsprite','grid_batches':8,'native_sizes':[r['raw_native_size'] for r in records],'final_size':[600,600],'native_size_note':'Final600pxiconcanvasisresampledfromactualnativegridcells;itdoesnotclaim600nativeAIpixelspericon.','atlas':{'path':png.relative_to(ROOT).as_posix(),'size':[7912,6088],'count':124,'frame_layout_preserved':True,'xml_unchanged':True,'sha256':sha(png)},'assets':checks},ensure_ascii=False,indent=2)+'\n',encoding='utf8')
