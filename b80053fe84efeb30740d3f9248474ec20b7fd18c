...
def main():
 ...
 paths=inventory(project);rows=[]
 for p in paths:
  before=p.stat();digest=sha(p);after=p.stat();require((before.st_size,before.st_mtime_ns)==(after.st_size,after.st_mtime_ns),'Input changed during hashing: '+str(p));rows.append({'path':p.relative_to(project).as_posix(),'sha256':digest,'bytes':after.st_size})
