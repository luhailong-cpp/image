...
def main():
    ...
    report={'schema':'qdao-festival-edge-final-audit-v1','status':'passed','checked_utc':datetime.now(timezone.utc).isoformat(),'scope':'v11 six-role RGB-only export repair and complete eight-role delivery ZIP','new_image_generation':False,
       ...
       'artifacts':[{ 'path':p.relative_to(ROOT).as_posix(),'sha256':sha(p)} for p in [ROSTER/'manifest.json',ROSTER/'validation.json',archive,ROSTER/'download.json',PACK/'visual-approval.json',PACK/'stage.json',PACK/'derived.json',PACK/'gif-validation.json',PACK/'publication.json',PACK/'before.json',PACK/'repair_v11_edges.py',PACK/'rebuild_delivery.py',PACK/'verify_rebuild.py',PACK/'supplement_28/approval.json',PACK/'supplement_28/publication.json',PACK/'supplement_28/repair.py']],
