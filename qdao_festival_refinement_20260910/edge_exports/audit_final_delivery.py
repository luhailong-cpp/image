...
def main():
    ...
    with zipfile.ZipFile(archive) as z:
        assert len(z.namelist())==download['entries']==738
    ...
    for f in publication['files']:assert sha(ROOT/f['path'])==f['sha256'],f['path']
    report={'schema':'qdao-festival-edge-final-audit-v1','status':'passed','checked_utc':datetime.now(timezone.utc).isoformat(),'scope':'v11 six-role RGB-only export repair and complete eight-role delivery ZIP','new_image_generation':False,
       'summary':{'formal_media_verified':len(checked),'changed_media':len(changed),'changed_by_extension':dict(Counter(Path(p).suffix for p in changed)),'retained_media':len(retained),'protected_27_30_media_unchanged':len(protected),'retained_23_24_29_portraits':len(retained_portraits),'zip_entries':download['entries'],'zip_formal_media_byte_identical':len(checked),'zip_metadata_and_visual_evidence_byte_identical':len(zip_extra),'new_contact_evidence':evidence,'immutable_before_files_hash_verified':len(before['files']),'published_records_verified':len(publication['files'])},
       'numeric_acceptance':load(PACK/'derived.json')['summary'],'visual_acceptance_record':'visual-approval.json','reproduction_trial':'verify_rebuild.py --trial passed 12 independent images; see README.md for full read-only command',
       'artifacts':[{ 'path':p.relative_to(ROOT).as_posix(),'sha256':sha(p)} for p in [ROSTER/'manifest.json',ROSTER/'validation.json',archive,ROSTER/'download.json',PACK/'visual-approval.json',PACK/'stage.json',PACK/'derived.json',PACK/'gif-validation.json',PACK/'publication.json',PACK/'before.json',PACK/'repair_v11_edges.py',PACK/'rebuild_delivery.py',PACK/'verify_rebuild.py']],
