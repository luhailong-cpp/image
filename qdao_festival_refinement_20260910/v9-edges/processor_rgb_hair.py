...
def clean(im, extra_mask=None):
    ...
    hair_donor=donor&(np.max(a[:,:,:3],axis=2)<105)
    h,w=alpha.shape; unresolved=[]; distances=[]; distant_examples=[]
    for y,x in zip(*np.nonzero(mask)):
        best=None
        for radius in [8,16,32,64,128]:
            x0=max(0,x-radius);x1=min(w,x+radius+1);y0=max(0,y-radius);y1=min(h,y+radius+1)
            active_donor=hair_donor if extra_mask is not None and extra_mask[y,x] else donor
            yy,xx=np.nonzero(active_donor[y0:y1,x0:x1]); yy+=y0;xx+=x0
            if len(xx):
                dd=(xx-x)**2+(yy-y)**2
        ...
        if distances[-1] > 32:
            distant_examples.append(dict(pixel=[int(x),int(y)],donor=[int(best[1]),int(best[0])],distance=distances[-1],before=a[y,x,:3].tolist(),after=out[y,x,:3].tolist()))
