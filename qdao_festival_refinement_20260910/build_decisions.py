...
for file in ['reviews/ui-items-old-hero-review.json','reviews/v11-style-review.json','reviews/v9-pets-style-review.json']:
...
publications=['prelogin/publication.json','prepared-sync/publication.json','scenes-sync/publication.json','scenes-sync/server-publication.json','edge_exports/publication.json','v9-edges/publication.json']
for file in publications:
 ...
 for r in j.get('files',[]):
  ...
  if p and expected:setrow(p,'updated','Current authorized refinement or synchronized derivative published and validated; see per-file provenance and backup.',file,expected)
