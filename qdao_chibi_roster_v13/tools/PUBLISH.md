...
The isolated tested project's complete Assets / Packages / ProjectSettings file set and every SHA must still match its snapshot; shared writable links and symlink/junction ancestors are rejected. The publication target must match the tested **relevant** input set and hashes: character catalog/animator and the two character importers (`Assets/Editor/QdaoCharacterSpriteImporter.cs`, `Assets/Editor/QdaoFrameAlphaProcessor.cs`, including their metadata), ActorWorld, GameClient, real movement controller, sandbox bootstrap, BattleArtCatalog, relevant Qdao/Battle appearance tests, assembly definitions, Packages/ProjectSettings and existing roster assets. Both removed and newly added relevant files are checked. Unrelated target UI/social/map differences are recorded in the receipt, remain untouched, and are not represented as validated by this run.
...
Observation idle semantics: all eight actors must report `actualHasDedicatedIdle:true` and `spriteMatchesDedicatedIdle:true`. Only Lu Dongbin is moved by this sandbox test, so only Lu must report `stoppedIdle:true` together with real movement. The other seven are inspected while idle and may correctly retain `movementObserved:false` and `stoppedIdle:false`; their raw Unity reports must not be edited.

Current baseline limitation (2026-09-17): the isolated run changed only `Assets/Resources/Fonts/QdaoBody SDF.asset`, a dynamic font asset. The original/current hashes are retained in `runtime-validation/summary.json` and `publication-current-limitations.json`. The publisher still verifies every private input byte and currently rejects that difference; no font exception, restoration, or concealed rewrite has been added. A future real-V13 validation must establish a stable input snapshot or a specifically evidenced policy for runtime font output before publication. UI editor tools `QdaoUguiBuilder`, `QdaoUiReadabilityVerification`, and `QdaoUiTextureQuality` are outside the character-import gate; their changes are recorded as unrelated and left untouched.
