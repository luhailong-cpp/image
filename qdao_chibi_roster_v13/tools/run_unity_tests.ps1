param([Parameter(Mandatory=$true)][ValidateSet('EditMode','PlayMode')][string]$Platform,[string]$Run='contract-run1',[Parameter(Mandatory=$true)][string]$InputSnapshot)
$ErrorActionPreference='Stop'
$v13Project='E:\work\tmp\qdao-v13-verify-20260917'
$v13Evidence=Join-Path 'E:\work\image\qdao_chibi_roster_v13\runtime-validation' $Run
...
[ordered]@{started_utc=$v13Start.ToString('o');pid=$v13Process.Id;project=$v13Project;platform=$Platform;filter=$v13Filter;arguments=$v13Arguments;capture_directory=$env:QDAO_ROSTER_CAPTURE_DIR;input_snapshot=$env:QDAO_ROSTER_INPUT_SNAPSHOT;input_snapshot_sha256=$v13SnapshotSha;scope='New V13 code with existing approved V12 artwork; synthetic test fixtures are not production art'} | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath (Join-Path $v13Evidence ($v13Stem+'-launch.json')) -Encoding utf8
