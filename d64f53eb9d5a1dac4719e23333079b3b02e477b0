...
a=json.loads((char/'source/assembly.json').read_text(encoding='utf8'))
...
m['layout_provenance']='source/assembly.json'
m['direction_reclassification']='source/SE-reclassification.txt' if (char/'source/SE-reclassification.txt').exists() else None
for kind in ['cardinal','diagonal']:
 ...
 m['sources'][kind]['original_sources']='source/assembly.json'
