...
sets=[]
for folder in ['processing-keys-final','processing-inbetweens-color']:
    base=HERE/folder
...
for i,f in enumerate(frames):
    ...
    d.text((x+25,y+20),f'NW-{i+1:02} | 60 ms',fill='#24362f')
...
manifest={'status':'direction_sample_pending_root_visual_review','direction':'NW','frame_count':8,'duration_ms':60,'unique_rgba_frames':8,'same_raw_cell_scale':True,'shared_final_scale':1.0,'published_to_game':False,'final_contract_unchanged':'8 directions x 8 authored walk frames + 8 independent idle frames','sources':[{'file':f,'sha256':hashlib.sha256((HERE/f).read_bytes()).hexdigest()} for f in ['keys-final-raw.png','inbetweens-color-raw.png']],'head_metrics':metrics,'frames':records}
