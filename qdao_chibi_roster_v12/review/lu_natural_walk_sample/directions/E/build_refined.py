...
# Each single was generated on a 1254 square canvas; resize the complete canvas
# to the native 627 grid cell, never fit an individual subject bounding box.
for i in range(4,8):
 source_single=HERE/f'single{i+1:02}-raw.png'
 single=Image.open(source_single).convert('RGB')
 assert single.size==(1254,1254)
 raw.paste(single.resize((627,627),Image.Resampling.LANCZOS),(i%4*627,i//4*627))
 records[i]={'frame':i+1,'source':str(source_single),'source_sha256':sha(source_single),'source_box':[0,0,1254,1254],'operation':'whole square canvas resized uniformly 0.5 to normalize source pixel resolution; no bbox fitting','whole_cell_scale':0.5}
rawpath=HERE/'walk-E-refined-raw.png' 
raw.save(rawpath)
(rawpath.with_suffix('.assembly.json')).write_text(json.dumps({'output_sha256':sha(rawpath),'frames':records},indent=2),encoding='utf-8')
frames,meta,source,errors=run_sheet(rawpath,'E',HERE/'processing-refined',padding=0,rows=2,cols=4)
assert not errors,errors
OUT=HERE/'preview-refined';OUT.mkdir(exist_ok=True)
