...
frames,meta,source,errors=run_sheet(HERE/'walk-W-raw.png','W',HERE/'processing',padding=0,rows=2,cols=4)
if errors: raise ValueError(errors)
OUT=HERE/'preview';OUT.mkdir(exist_ok=True)
records=[];final=[]
for i,frame in enumerate(frames):
 im,record=normalize(frame,1.0,despill_edges=True)
 im.save(OUT/f'W-{i+1:02}.png');final.append(im);records.append(record)
assert len({hashlib.sha256(im.tobytes()).hexdigest() for im in final})==8
assert all(body_ground_anchor(im)==(256.,471) for im in final)
compose(final,8).save(OUT/'W-strip.png');compose(final,4).save(OUT/'W-sheet.png')
proc=load_processor(DEFAULT_PROCESSOR);proc.save_transparent_gif(final,OUT/'W-transparent.gif',60)
for label,color in [('light','#e8e3d9'),('dark','#263737')]:
 ...
 mats[0].save(OUT/f'W-{label}.gif',save_all=True,append_images=mats[1:],duration=60,loop=0,disposal=2)
board=Image.new('RGB',(1536,870),'#e8e3d9');d=ImageDraw.Draw(board)
for i,im in enumerate(final):
 x=i%4*384;y=i//4*420
 thumb=im.resize((384,384),Image.Resampling.LANCZOS);board.paste(thumb,(x,y+45),thumb);d.text((x+25,y+20),f'W-{i+1:02} | 60 ms',fill='#24362f')
...
manifest={'status':'numeric_pass_pending_root_visual_review','direction':'W','frame_count':8,'duration_ms':60,'unique_rgba_frames':8,'same_raw_cell_scale':True,'shared_final_scale':1.0,'published_to_game':False,'source':source,'numeric_qc':meta['direction_qc'],'head_metrics':metrics,'frames':records}
