...
names=['phase01-single.png','agent-phase02-single.png','agent-phase03-single.png','agent-phase04-head-fixed.png','agent-phase05-canonical.png','agent-phase06-canonical.png','agent-phase07-single.png','agent-phase08-single.png']
...
rawpath=HERE/'walk-NE-canonical-raw.png';raw.save(rawpath)
rawpath.with_suffix('.assembly.json').write_text(json.dumps({'output_sha256':sha(rawpath),'frames':records},indent=2),encoding='utf-8')
frames,meta,source,errors=run_sheet(rawpath,'NE',HERE/'agent-canonical-processing',padding=0,rows=2,cols=4)
assert not errors,errors
OUT=HERE/'agent-canonical-preview';OUT.mkdir(exist_ok=True)
aligned=[];norms=[]
