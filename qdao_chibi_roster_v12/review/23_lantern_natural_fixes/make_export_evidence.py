...
import hashlib,json
R=Path(__file__).resolve().parent;ROOT=R.parent.parent;C=ROOT/'candidate-stable-body/23_lantern_courier';O=ROOT/'23_lantern_courier';D=['N','NE','E','SE','S','SW','W','NW'];EV=R/'export-evidence';EV.mkdir(exist_ok=True)
...
summary={'character_id':C.name,'status':'passed_exports_pending_visual','published':False,'sealed':False,'candidate_path':str(C),'alignment_version':3,'canonical_first_contact':'anatomical_RIGHT','phase_plan_sha256':sha(R/'phase-plan.json'),'common_scale':1.0194174757281553,'movement_frames':64,'neutral_idle_frames':8,'portrait_unchanged':True,'unchanged_raw_cells':53,'repainted_cells':19,'validation_sha256':sha(C/'validation.json'),'manifest_sha256':sha(C/'manifest.json'),'qc_sha256':sha(C/'qc.json'),'provenance_sha256':sha(R/'generation-provenance.json'),'retention_sha256':sha(R/'source-retention.json'),'qc_status':qc['status'],'verifier_status':val['status'],'original_directory_files_unchanged':True,'evidence':[{'path':str(p),'sha256':sha(p)} for p in sorted(EV.glob('*')) if p.is_file()]}
