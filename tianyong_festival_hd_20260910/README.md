...
本目录的高清地图美术资源已完成：36张原生1254分区重绘拼接为6144×6144总图，并导出36张1024切片。**未替换游戏生产图块、导航或遮挡，未完成运行集成。**

`layout-guide-6144.png` 和 `guides/` 只是由 1254 × 1254 参考稿放大的布局辅助图，不是高清成品。最终主图已使用 36 张独立细化的原生 PNG；本脚本不调用生图，不绘制创作内容。
