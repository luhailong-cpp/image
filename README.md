...
**主城高清重制规范：[Image 切图.md](Image切图.md)**。单个正式图块为 4096×4096，整图按最终行列拼合，不预设 16K；七套外观全部最高画质，客户端按完整视口与周边一圈加载。本轮已建立制作与验收合同，新的高清重绘尚未生成，模型调用状态见规范。

2026-09-14 [群组、世界频道与谣言 UI](designs/social-ui-v1/README.md)：已补齐三张同风格效果图与可交互页面，群列表／群聊／资料／邀请、四个聊天频道与只读系统传闻均可演示，附桌面和手机验收。

2026-09-14 [仙笺邮件 UI](designs/mail-ui-v1/README.md)：道家 Q 版活动邮件与普通邮件两张效果图、独立中秋活动横幅和原生文字交互预览，包含附件领取、通知、已领与过期状态。

2026-09-13 组队 UI 重做：[新版效果图与交互预览](designs/team-ui-v2/README.md)。成员名单与申请列表并排呈现，按用户要求去掉“已同意”；放大头像、姓名与状态，保留同意／拒绝及权限提示。已接入实际 Unity 客户端：27 项 EditMode、6 项 PlayMode 与 20 张实际截图验收通过；线上组队仍待服务端接口接入。

2026-09-13 [帮会 UI 续作](designs/guild-ui-v2/README.md)：已补齐六页原生文字交互预览与27张独立手绘资源，成员筛选、签到、捐献、领奖、报名、兑换及持久化通过浏览器验收。原总览静态图保留在旧图库；已接入 Unity「帮会 [G]」入口，27项定向测试及20张实际截图通过；联机范围见续作交付记录。

最新场景交付：[高清主城与五类场景重绘（已完成）](SCENE_DELIVERY.md)。包含6144主城总图、36张地图切片及统一Q版道家节庆风格的场景成图。
...
2026-09-12 全库节庆精修已完成：[最终交付与逐文件验收](qdao_festival_refinement_20260910/README.md)。整体保持道家 Q 版，春节／元宵／中秋适量点缀；登录前整屏、人物边缘、场景与准备副本已更新，待处理项为 0。长期风格继续遵守[节庆方案](docs/QDAO_FESTIVAL_REFINEMENT.md)。
