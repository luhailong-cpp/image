"""Build W-only static review sheets from distinct native generated frames."""
...
def main():
 ...
 rows=[];images=[]
 for n in range(1,17):
  archive=GEN/f'W{n:02d}-single-v{VERSIONS.get(n,1)}';candidate=archive/'staging/candidate/05_celestial_musician_girl';output=candidate/f'walk/W/{n:02d}.png'
  ...
  rec=json.loads((candidate/'processing/frame-sources.json').read_text())[f'walk/W/{n:02d}.png'];assert rec['common_scale']==.84 and rec['chroma_thresholds']==[50,75]
  rows.append({'frame':n,'slot':f'walk/W/{n:02d}.png','output':str(output),'sha256':sha(output),'raw':str(archive/'raw.png'),'rawSha256':sha(archive/'raw.png'),'generationRecord':str(archive/'raw.png.generation.json'),'prompt':str(archive/'prompt.txt'),'receipt':str(archive/'generation-receipt.json'),'sourceMapping':str(candidate/'processing/frame-sources.json'),'validation':str(candidate/f'review/validation-W-{n:02d}.json'),'nativeSize':rec['source']['native_size'],'outputSize':[1024,1024],'alphaBbox':[int(xx.min()),top,int(xx.max()+1),bottom+1],'visibleHeight':height,'anchor':[axis,bottom],'pixelSha256':hashlib.sha256(im.tobytes()).hexdigest(),'commonScale':.84,'chromaThresholds':[50,75],'actualModel':None,'actualQuality':None,'visualReview':'static_pending_root_loop'})
  images.append((n,im))
 if a.complete:assert len(rows)==16,'All 16 W frames are required'
 for name,color,ink in [('dark','#202b38','white'),('light','#f0eee4','black')]:
  for kind in ['normal','legs']:
   ...
   for i,(n,im) in enumerate(images):
    ...
    x=(i%4)*tilew;y=(i//4)*tileh;sheet.paste(content,(x,y+30),content);ImageDraw.Draw(sheet).text((x+8,y+8),f'W{n:02d} / v{VERSIONS.get(n,1)}',fill=ink)
 ...
 report={'character':'05_celestial_musician_girl','direction':'W','at':datetime.now(timezone.utc).isoformat(),'frameCount':len(rows),'frameDurationMs':30,'complete':len(rows)==16,'sourceDistinctCount':len({r['rawSha256'] for r in rows}),'pixelDistinctCount':len({r['pixelSha256'] for r in rows}),'rows':rows,'approval':False,'status':'static_candidates_pending_root_loop','canonicalModified':False}
 write(OUT/'snapshot.json',report)
 if a.complete:
  frames=[]
  for n,im in images:
   frame=Image.new('RGB',(512,544),'#202b38');small=im.resize((512,512),Image.Resampling.LANCZOS);frame.paste(small,(0,32),small);ImageDraw.Draw(frame).text((12,10),f'W{n:02d} / 30ms',fill='white');frames.append(frame)
  for ext in ['gif','webp']:
   frames[0].save(OUT/f'W-16-30ms.{ext}',save_all=True,append_images=frames[1:],duration=30,loop=0,**({'disposal':2} if ext=='gif' else {'lossless':True}))
  for name,color in [('dark','#202b38'),('light','#f0eee4')]:
   seam=Image.new('RGB',(2048,544),color)
   for x,n in enumerate((15,16,1,2)):
    im=dict(images)[n].resize((512,512),Image.Resampling.LANCZOS);seam.paste(im,(x*512,32),im);ImageDraw.Draw(seam).text((x*512+12,12),f'W{n:02d}',fill='white' if name=='dark' else 'black')
