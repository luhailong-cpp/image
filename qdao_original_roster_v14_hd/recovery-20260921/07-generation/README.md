...
> 2026-09-23保留规则更新：用户明确要求只保留接入用图和设计。已删除选用稿原始输出、重复归档、拒稿及回退图片，文字来源记录保留；只有尚无导出成品的唯一在制稿暂存供继续制作。本更新覆盖下文历史“raw/拒稿永久保留”的描述。准确清理清单见`../07-tools/cleanup-20260923.json`，当前图片在`../07-tools/candidate/07_moon_shadow_assassin_girl/`。目前仅候选，未完成全角色美术验收。
