...
2026-09-23 保留策略更新：用户已明确授权删除原图、回退及拒稿；按最新 `AGENTS.md`，只保留最终游戏资源、设计、必要说明与尚未导出成品的唯一在制稿。`../10-work/cleanup-20260923/plan.json` 是精确清单，两份 `result-*.json` 记录实际删除并复核的 104 个文件（142,794,485 字节）。已移除旧预览的 contact 链接。44 张唯一原生在制稿暂留，不代表美术通过；最终导出后继续清理。

以下归档说明中的原始默认路径和已删拒稿路径属于历史来源证据。旧 `static-check.json` 只证明归档时的核验结果；清理后不能将旧检查当作当前文件仍存在的证明。不要改写旧回执、重新制造 raw 或为已删文件伪造新通过记录。
