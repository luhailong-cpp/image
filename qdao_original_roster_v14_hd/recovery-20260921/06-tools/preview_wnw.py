"""Render factual previews for isolated W/NW source pixels; never fills missing slots."""
from pathlib import Path
from datetime import datetime,timezone
import argparse,hashlib,json
from PIL import Image,ImageDraw
HERE=Path(__file__).resolve().parent;WORK=HERE/'work-W-NW/candidate/06_thunder_caster_boy'
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
p=argparse.ArgumentParser();p.add_argument('--revision',required=True);p.add_argument('--direction',choices=['W','NW','both'],default='both');a=p.parse_args()
OUT=HERE/'wnw-previews'/a.revision;assert not OUT.exists();OUT.mkdir(parents=True)
dirs=['W','NW'] if a.direction=='both' else [a.direction];rows=[];gifs=[]
for d in dirs:
...
manifest={'created_at_utc':datetime.now(timezone.utc).isoformat(),'pngs':rows,'gifs':gifs,'missing':[f'{d}{n:02d}' for d in dirs for n in range(1,17) if not (WORK/f'walk/{d}/{n:02d}.png').exists()],
          ...
          'NW_phase_offset':'01 left contact,05 right swing,09 right contact,13 left swing; chosen from observed NW01 pose, not prompt assumption'}
(OUT/'manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
html='<!doctype html><meta charset="utf-8"><title>06 W/NW 30ms previews</title><style>body{background:#eee9dc;color:#163831;font:18px system-ui;margin:24px}img{width:512px;max-width:95vw}a{color:#175d57}</style><h1>06 雷法少年 W / NW 独立候选</h1><p>16帧 × 30ms = 480ms。每帧真实独立原图；美术/动态最终验收待root。</p>'
for d in dirs:
    html+=f'<h2>{d}</h2>'
