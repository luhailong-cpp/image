"""Generic original-Q roster V14 HD deterministic processing. No code-generated artwork."""
...
ROOT=Path(__file__).resolve().parents[2]; FRAME=1024; ROOT_X=512; ROOT_Y=942; DIRS=('N','NE','E','SE','S','SW','W','NW')
...
def import_sheet(a):
 native_alpha=getattr(a,'chroma_profile','standard')=='native-alpha'
 chroma_thresholds=(0,0) if native_alpha else (50,75) if getattr(a,'chroma_profile','standard')=='purple-preserve' else (100,150)
 ...
 profile_path=out/'processing/scale-profile.json';scale=float(a.common_scale if a.common_scale is not None else load(profile_path)['common_scale'] if profile_path.exists() else .84 if a.character.startswith(('04_','05_')) else .88)
 require(math.isfinite(scale) and scale>0,'Invalid character-wide scale')
 raw=imread(a.source);require(not native_alpha or (np.asarray(raw)[:,:,3].min()==0 and np.asarray(raw)[:,:,3].max()==255),'native-alpha requires genuine transparent background and opaque subject');cw=raw.width/cols;ch=raw.height/rows;factor=FRAME/max(cw,ch)*scale
 require(min(cw,ch)>=1024,'Native generated cell must be at least1024 in BOTH dimensions; low-resolution source cannot be upscaled into HD')
 require(factor<=1,'HD whole-cell upscaling is forbidden')
 for index in source_indices:
  row,col=divmod(index,cols);box=[round(col*cw),round(row*ch),round((col+1)*cw),round((row+1)*ch)]
  require(min(box[2]-box[0],box[3]-box[1])>=1024,'Rounded native cell is below1024')
 profile={'whole_cell_normalization':'1024/max(native_cell_width,native_cell_height)','common_scale':scale,'per_subject_bbox_scaling':False,'root_px':[512,942],'alignment_version':2,'horizontal':'upper_body_alpha_median_42_percent','vertical':'lowest_alpha_gt_8'}
 ...
 raw=imread(source_dir/'raw.png');cw=raw.width/cols;ch=raw.height/rows;factor=FRAME/max(cw,ch)*scale
 ...
 for index in source_indices:
  ...
  box=[round(c*cw),round(r*ch),round((c+1)*cw),round((r+1)*ch)];cell=raw.crop(box);prepared=cell.copy();alpha_cleanup_threshold=0 if native_alpha else 8 if np.asarray(raw)[:,:,3].min()<255 else 0
  ...
  keyed=prepared.copy() if native_alpha else KEYER.remove_bg_magenta(prepared,*chroma_thresholds)
  native_source_boundary_max=max(int(np.asarray(keyed)[:,:,3][0].max()),int(np.asarray(keyed)[:,:,3][-1].max()),int(np.asarray(keyed)[:,:,3][:,0].max()),int(np.asarray(keyed)[:,:,3][:,-1].max()))
  require((native_alpha and native_source_boundary_max<=8) or not edge_touch(keyed),f'{d}/{frame:02d}: original visible cell touches boundary; regenerate rather than hide clipping')
  ...
  clean,cleanup=(normal.copy(),{'mode':'native-rgba-preserved','alpha_equal':True,'rgb_equal':True,'changed_pixels':0,'protected_red_changes':0,'outside_band_changes':0}) if native_alpha else EDGE.despill(normal,radius=4,reference_radius=12);ax,ay=axis(clean);delta=[round(ROOT_X-ax),ROOT_Y-ay];b=cropbox(clean,0)
  moved=[b[0]+delta[0],b[1]+delta[1],b[2]+delta[0],b[3]+delta[1]]
  require(min(moved[:2])>=1 and max(moved[2:])<=FRAME-1,f'{d}/{frame:02d}: aligned frame would clip {moved}; correct generation or one common character-wide scale')
  final=Image.new('RGBA',(FRAME,FRAME));final.paste(clean,tuple(delta));relative=f'walk/{d}/{frame:02d}.png' if kind=='walk' else f'idle/{d}.png'
  stages={}
  ...
  rec={'kind':kind,'direction':d,'frame':frame,'source':{'path':(source_dir/'raw.png').relative_to(out).as_posix(),'sha256':sha(source_dir/'raw.png'),'native_size':list(raw.size),'grid':[rows,cols],'cell_index':index,'sequence_start_frame':output_frames[source_indices[0]] if kind=='walk' else 0,'selected_source_cell_indices':source_indices,'output_frame_map':output_frames if kind=='walk' else None,'output_direction_map':list(idle_order) if kind=='idle' else None,'cell_xyxy':box},'prompt':{'path':(source_dir/'prompt.txt').relative_to(out).as_posix(),'sha256':sha(source_dir/'prompt.txt')},'generation':{'tool':'built-in image_gen',**generation_request(receipt),'model_actual':(load(receipt).get('actual_model') or load(receipt).get('model_actual') or 'host-managed-unverified') if receipt.exists() else 'host-managed-unverified','receipt':{'path':receipt.relative_to(out).as_posix(),'sha256':sha(receipt)} if receipt.exists() else None,'status':'unreviewed'},'whole_cell_scale':factor,'common_scale':scale,'normalized_size':list(size),'chroma_thresholds':list(chroma_thresholds),'chroma_profile':'native-alpha' if native_alpha else getattr(a,'chroma_profile','standard'), 'pipeline_variant':'06-isolated-native-alpha-v1' if native_alpha else 'v14-standard','native_source_boundary_alpha_max':native_source_boundary_max,'native_alpha_low_boundary_pixels_retained':native_alpha,'alpha_cleanup_threshold':alpha_cleanup_threshold,'cleanup':cleanup,'despill_radius':0 if native_alpha else 4,'translation_px':delta,'alignment_version':2,'anchor_after_px':list(axis(final)),'stages':stages,'output':relative,'output_sha256':stages['final']['sha256'],'visual_review':'pending'}
...
def review(character):
 ...
 for d in DIRS:
  ...
  if not all(p.exists() for p in paths):
   ...
   if available:
    rows=math.ceil(len(available)/4);sheet=Image.new('RGB',(4096,rows*1088),(38,44,45));draw=ImageDraw.Draw(sheet)
    for slot,(number,im) in enumerate(available):
     x=(slot%4)*FRAME;y=(slot//4)*1088;sheet.paste(im,(x,y),im);draw.text((x+12,y+1028),f'{d} / {number:02d} [partial]',fill='white')
    save(sheet,out/f'review/{d}-partial-contact.png')
   continue
  frames=[imread(p) for p in paths];strip=Image.new('RGBA',(16*FRAME,FRAME))
  for i,im in enumerate(frames):strip.paste(im,(i*FRAME,0))
  save(strip,out/f'review/strips/{d}.png')
  idle=imread(out/f'idle/{d}.png') if (out/f'idle/{d}.png').exists() else None
  display=([idle] if idle is not None else [])+frames;sheet=Image.new('RGB',(4096,5*1088 if idle else 4*1088),(38,44,45));draw=ImageDraw.Draw(sheet)
  for i,im in enumerate(display):
   x=(i%4)*FRAME;y=(i//4)*1088;sheet.paste(im,(x,y),im);draw.text((x+12,y+1028),f'{d} / '+('idle' if idle is not None and i==0 else f'{i if idle else i+1:02d}'),fill='white')
  ...
  for im in frames+([idle] if idle else []):
   x,y=axis(im)
   if abs(x-ROOT_X)>.5 or y!=ROOT_Y:problems.append('body_axis_or_feet_anchor')
 ...
 expected=['portrait.png',*[f'idle/{d}.png' for d in DIRS],*[f'walk/{d}/{i:02d}.png' for d in DIRS for i in range(1,17)]]
 files=[{'path':p,'sha256':sha(out/p)} for p in expected if (out/p).exists()];status='failed' if errors else ('passed_numeric_pending_visual' if len(files)==137 else 'incomplete')
 qc={'status':status,'visual_review':'pending','directions':directions,'cross_direction_mean_height_ratio':ratio,'errors':errors,'gates':{'body_scale_cv_max':.08,'idle_walk_height_drift_max':.08,'cross_direction_height_ratio_max':1.10,'unique_frames_per_direction':16,'horizontal_axis_error_px':.5,'foot_y':ROOT_Y}}
 write(out/'qc.json',qc)
 manifest={'version':14,'frame_size':[1024,1024],'portrait_size':[1024,1024],'frame_size_px':1024,'native_cell_min_px':1024,'pixels_per_unit':104,'runtime_geometry':{'reference_frame_size':512,'pixels_per_unit':104,'pivot':[.5,.08]},'character_id':character,'status':status,'visual_review':'pending','art_source':'built-in image_gen for every new walk/idle; original restored portrait','directions':list(DIRS),'frame_count':16,'frame_duration_ms':30,'cycle_duration_ms':480,'dedicated_idle':True,'contact_frame':0,'alignment':load(out/'processing/scale-profile.json') if (out/'processing/scale-profile.json').exists() else None,'sources_path':'processing/frame-sources.json','sources_sha256':sha(out/'processing/frame-sources.json') if sources else None,'files':files,'review_files':[{'path':f'review/strips/{d}.png','sha256':sha(out/f'review/strips/{d}.png')} for d in DIRS if (out/f'review/strips/{d}.png').exists()],'generated_walk_count':sum(r['kind']=='walk' for r in sources.values()),'generated_idle_count':sum(r['kind']=='idle' for r in sources.values())}
