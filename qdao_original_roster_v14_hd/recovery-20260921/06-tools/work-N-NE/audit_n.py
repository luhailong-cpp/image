...
result=verify.verify('06_thunder_caster_boy','N',False,None);write(REVIEW/'numeric-verification.json',result)
frames=[Image.open(OUT/f'walk/N/{i:02d}.png').convert('RGBA') for i in range(1,17)]
files=[]
for i,im in enumerate(frames,1):
 a=np.asarray(im)[:,:,3];y,x=np.where(a>8);files.append({'frame':i,'path':str(OUT/f'walk/N/{i:02d}.png'),'sha256':sha(OUT/f'walk/N/{i:02d}.png'),'size':list(im.size),'alpha_bbox_gt8':[int(x.min()),int(y.min()),int(x.max())+1,int(y.max())+1],'edge_alpha_nonzero':int(np.count_nonzero(a[0])+np.count_nonzero(a[-1])+np.count_nonzero(a[:,0])+np.count_nonzero(a[:,-1]))})
gif_results=[]
for theme,color,fg in [('dark',(28,36,44),(255,255,255)),('light',(244,240,228),(15,25,30))]:
 compos=[]
 for i,im in enumerate(frames,1):
  canvas=Image.new('RGB',(512,548),color);canvas.paste(im.resize((512,512),Image.Resampling.LANCZOS),(0,30),im.resize((512,512),Image.Resampling.LANCZOS));ImageDraw.Draw(canvas).text((12,10),f'N {i:02d}/16  30ms / 480ms loop',fill=fg);compos.append(canvas)
 gif=REVIEW/f'N-{theme}-30ms.gif';compos[0].save(gif,save_all=True,append_images=compos[1:],duration=30,loop=0,disposal=2,optimize=False)
 ...
 for j,im in enumerate(frames):
  small=im.resize((256,256),Image.Resampling.LANCZOS);x=(j%4)*256;y=(j//4)*288;contact.paste(small,(x,y+28),small);ImageDraw.Draw(contact).text((x+12,y+8),f'N{j+1:02d}',fill=fg)
 contact.save(REVIEW/f'N-contact-{theme}.png')
 seam=Image.new('RGB',(2048,1100),color)
 for j,n in enumerate([15,16,1,2]):
  im=frames[n-1].resize((512,512),Image.Resampling.LANCZOS);x=j*512;seam.paste(im,(x,32),im);ImageDraw.Draw(seam).text((x+20,8),f'N{n:02d}',fill=fg)
  crop=frames[n-1].crop((270,680,880,1010)).resize((512,277),Image.Resampling.LANCZOS);seam.paste(crop,(x,640),crop)
 seam.save(REVIEW/f'N-seam-{theme}.png')
