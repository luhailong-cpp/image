"""Prepare an exact real-call request for one independent W/NW sprite; does not generate."""
...
def main():
    ...
    parser.add_argument("direction", choices=("W", "NW"))
    ...
    refs = [RECOVERY / "20-reference/identity-inspection-1024.png",
            ...
            RECOVERY / ("20-generation/W-01-v2/raw.png" if direction == "W" else "20-generation/idle-S-v1/raw.png")]
    assert all(path.is_file() for path in refs)
    facing = ("WEST / W: LEFT-facing SIDE PROFILE, nose and toes point LEFT, body and pelvis face LEFT. Anatomical LEFT side is nearest camera, near LEFT hand holds three star cards, far RIGHT hand holds the circular astrolabe. Preserve arms and hands even with natural occlusion."
              if direction == "W" else
              "NORTHWEST / NW: back-left THREE-QUARTER view, heading diagonally toward the TOP LEFT of the image. We predominantly see her BACK and left side, only a tiny left cheek contour if naturally visible, no front-facing eyes. Her anatomical LEFT hand on the near left side holds exactly three star cards, her anatomical RIGHT hand on the far right side holds the astrolabe, with physically correct natural occlusion. Do not flip or swap them.")
