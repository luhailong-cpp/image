...
ROOT=Path(__file__).resolve().parents[1]; FRAME=1024; ROOT_X=512; ROOT_Y=942; DIRS=('N','NE','E','SE','S','SW','W','NW')
...
def import_sheet(a):
 ...
 raw=imread(a.source);cw=raw.width/cols;ch=raw.height/rows;factor=FRAME/max(cw,ch)*scale
 ...
 for index in source_indices:
  ...
  clean,cleanup=EDGE.despill(normal,radius=4,reference_radius=12);ax,ay=axis(clean);delta=[round(ROOT_X-ax),ROOT_Y-ay];b=cropbox(clean,0)
