...
EXPECTED_OUTPUTS = EXPECTED_PNGS | {'manifest.json', 'validation.json', 'appearance.json'}
CAMERA_CONFIG = 'Assets/Resources/World/Tianyong/TianyongMapConfig.asset'
CAMERA_INPUT_PATHS = (CAMERA_CONFIG, CAMERA_CONFIG + '.meta',
...
HD_REQUIRED_METHODS = {
    'EditMode': ('MmorpgClient.Tests.EditMode.Tianyong.QdaoOriginalHdAppearanceTests', {
        'CompleteHdHas137Separate1024ImagesAndPreservesWorldSizeCadenceAndPivot': 1,
        'WrongHdActivationCannotBecomeAvailable': 8, 'WrongHdManifestCannotBorrowAValidApproval': 5,
        'EveryMissingHdImageFallsBackAtomicallyToSameIdentityV13AndNeverAnUnrelatedBody': 1,
        'MissingHdIndexDoesNotProbeTexturePixelsAndImportCompletionUpgradesFallback': 1,
        'ActualLoadRejectionKeepsAllConsumersOnSameIdV13UntilImportRefresh': 1,
        'OldActorLoadFailureCannotRejectANewerAcceptedHdRevision': 1,
        'DerivedIndexRejectsStaleRevisionDuplicateMissingAndChangedDimensionsWithoutTextureLoads': 1,
        'IndexHasNoTextureOrObjectReferenceFieldsAndMissingIndexNeverLoadsArtwork': 1,
    }),
    'PlayMode': ('MmorpgClient.Tests.PlayMode.QdaoHdResourceLifetimePlayModeTests', {
        'HdLoadsOneDirectionAndOneObservationWithoutChangingVisibleSprite': 1,
        'HdAllDirectionsKeepThirtyMillisecondCadenceAndStopOnIndependentIdle': 1,
        'SwitchingAllTwentyThreeIdentitiesAndDirectionsDoesNotAccumulateInactiveHdFrames': 1,
        'PartialOrWrongSizeHdDirectionReleasesEveryAcquiredTexture': 5,
        'MissingHdDirectionFallsBackAtomicallyToSameIdV13': 1,
        'FailedFirstHdLoadWithoutFallbackPreservesTheExistingActorVisual': 1,
        'NewRevisionSharingTexturesCannotBeUnloadedByRetiringTheOldRevision': 1,
        'RealBattleViewAndAfterimageKeepTheirSpritesUntilTheirOwnLastLeaseEnds': 1,
        'PreparingAnIdleSpriteKeepsTheDisplayedImageAliveUntilTheSwapIsBound': 1,
        'BattleMissingHdFrameReturnsOnlySameIdentityLegacyFramesOrNothing': 1,
    }),
}
...
def check_results(path, required_suffix=None, hd_platform=None):
    ...
    names = [test.get('fullname') for test in cases]
    require(result.get('result') == 'Passed' and all(int(result.get(key, '-1')) == 0
            for key in ('failed', 'skipped', 'inconclusive')) and cases and
            int(result.get('passed', '0')) == int(result.get('total', '0')) == len(cases) and
            all(test.get('result') == 'Passed' for test in cases) and all(names) and len(set(names)) == len(names),
            'Unity result must include only distinct passed cases, with no skips/inconclusive results')
    if required_suffix:
        require(any(name.endswith(required_suffix) for name in names),
                'Required actual Unity test was not passed: ' + required_suffix)
    if hd_platform:
        class_name, methods = HD_REQUIRED_METHODS[hd_platform]
...
def utc(value):
    ...
    require(parsed.tzinfo is not None, 'Evidence timestamps must identify UTC offset')
    return parsed.timestamp()


def check_runtime_inventory(rows, plan, require_index=False):
    require(set(plan['outputs']) == EXPECTED_OUTPUTS and len(plan['outputs']) == 140,
            'Publication must contain exactly137 PNGs +3 JSON files')
    prefix = FAMILY + '/' + plan['characterId'] + '/'
    relative = {path[len(prefix):]: row for path, row in rows.items() if path.startswith(prefix)}
    directory_metas = {'idle.meta', 'walk.meta'} | {'walk/' + direction + '.meta' for direction in DIRECTIONS}
    allowed = EXPECTED_OUTPUTS | {path + '.meta' for path in EXPECTED_OUTPUTS} | directory_metas | {
        'runtime-index.asset', 'runtime-index.asset.meta'}
    require(set(relative) <= allowed and EXPECTED_OUTPUTS <= set(relative),
            'Tested/target HD authored inventory must be exactly140 files; extra, orphan or nested index files are forbidden')
    for path, digest in plan['outputs'].items():
        require(relative[path]['sha256'] == digest, 'HD authored bytes differ: ' + prefix + path)
    if require_index:
        require({'runtime-index.asset', 'runtime-index.asset.meta'} <= set(relative) and
                {path + '.meta' for path in EXPECTED_OUTPUTS} <= set(relative),
                'Play input requires the actual generated index and all authored asset GUID metas')


def check_launch_binding(xml_path, report, rows, plans, expected_snapshot_sha=None, platform=None):
    require(platform in EXPECTED_FILTERS, 'An explicit Unity platform is required')
    xml_path = Path(xml_path)
    launch_path = xml_path.with_name(xml_path.stem + '-launch.json')
    completion_path = xml_path.with_name(xml_path.stem + '-completion.json')
    expected_log = xml_path.with_suffix('.log')
    launch, completion = read(launch_path), read(completion_path)
    require(launch.get('platform') == platform and launch.get('filter') == EXPECTED_FILTERS[platform],
            'Unity launch must run the complete expected platform filter')
    require(type(completion.get('exit_code')) is int and completion['exit_code'] == 0 and
            completion.get('xml_exists') is True and Path(completion['log']).resolve() == expected_log.resolve() and
            expected_log.is_file() and expected_log.stat().st_size > 0, 'Unity must finish with exit0 and retained actual log/XML')
    arguments = launch.get('arguments', [])
    def flag(name):
        require(arguments.count(name) == 1 and arguments.index(name) + 1 < len(arguments), 'Unity launch flag missing/duplicated: ' + name)
        return arguments[arguments.index(name) + 1].strip('"')
    require(flag('-testPlatform') == platform and flag('-testFilter') == EXPECTED_FILTERS[platform] and
            Path(flag('-testResults')).resolve() == xml_path.resolve() and
            Path(flag('-logFile')).resolve() == expected_log.resolve() and
            Path(flag('-projectPath')).resolve() == Path(launch['project']).resolve(), 'Unity command and evidence destinations differ')
    ...
    require(launched_snapshot.get('shared_writable_links') is False, 'Unity input must not share writable source links')
    ...
    start, end = utc(result.attrib['start-time']), utc(result.attrib['end-time'])
    require(utc(launch['started_utc']) <= start + 1 and start <= end and end <= utc(completion['finished_utc']) + 1,
            'Unity launch/completion timestamps do not enclose its actual test run')
    if expected_snapshot_sha is not None:
        generated = utc(report['generatedUtc'])
        require(start <= generated <= end + 1, 'Runtime observation timestamp is outside supplied PlayMode test run')
    return launch
...
def near(actual, expected, tolerance, label):
    require(finite_number(actual) and abs(actual - expected) <= tolerance, label + ' differs')


def camera_contract(project):
    raw = safe_child(project, CAMERA_CONFIG).read_text(encoding='utf-8-sig')
    def field(name):
        values = re.findall(r'^\s*' + re.escape(name) + r':\s*([^\r\n]+)\s*$', raw, re.M)
        require(len(values) == 1, 'Camera configuration field missing/duplicated: ' + name)
        value = float(values[0]); require(math.isfinite(value), 'Camera configuration must be finite')
        return value
    minimum, maximum, default = field('cameraZoomMin'), field('cameraZoomMax'), field('cameraZoomDefault')
    minimum = minimum if minimum >= 1 else 5
    maximum = max(minimum, maximum if maximum >= 1 else 30)
    default = min(maximum, max(minimum, default if default >= 1 else 27))
    return {'minimum': minimum, 'default': default}


def check_runtime_view(actor, view_name, capture_root, camera):
    view = actor.get(view_name)
    require(isinstance(view, dict) and isinstance(view.get('imagePath'), str) and view['imagePath'],
            'Actual runtime screenshot path is missing: ' + view_name)
    suffix = '-nearest-zoom' if view_name == 'nearestView' else ''
    path = safe_child(capture_root, 'tianyong-' + actor['actualCharacterId'] + suffix + '.png')
    require(Path(view['imagePath']).is_absolute() and Path(view['imagePath']).resolve() == path.resolve() and
            path.is_file() and path.stat().st_size > 100000, 'Actual runtime screenshot is missing, outside the run, or empty')
    digest = view.get('imageSha256')
    require(isinstance(digest, str) and re.fullmatch('[0-9a-f]{64}', digest) and sha(path) == digest,
            'Actual runtime screenshot bytes changed or SHA is absent')
    with Image.open(path) as image:
        require(image.format == 'PNG' and image.size == (1920, 1080), 'Runtime screenshot must really be a1920x1080 PNG')
        image.load()
    require(view.get('renderWidth') == 1920 and view.get('renderHeight') == 1080, 'Runtime screenshot dimensions disagree with observation')
    near(view.get('configuredZoomMin'), camera['minimum'], .0001, 'Captured minimum zoom configuration')
    near(view.get('configuredZoomDefault'), camera['default'], .0001, 'Captured default zoom configuration')
    expected_zoom = camera['minimum'] if view_name == 'nearestView' else camera['default']
    near(view.get('requestedZoom'), expected_zoom, .0001, 'Requested actual camera zoom')
    near(view.get('actualOrthographicSize'), expected_zoom, .0001, 'Actual camera zoom')
    bounds = [view.get(key) for key in ('frameLeftPixels', 'frameRightPixels', 'frameBottomPixels', 'frameTopPixels')]
    require(all(finite_number(value) for value in bounds), 'Runtime projected bounds must be finite')
    left, right, bottom, top = bounds
    require(left < right and bottom < top and right > 0 and left < 1920 and top > 0 and bottom < 1080,
            'The projected character does not intersect the real screenshot')
    near(view.get('projectedFrameHeightPixels'), top - bottom, .02, 'Projected frame bounds')
    near(view.get('projectedFrameHeightPixels'), actor['actualFrameWorldHeight'] * 1080 / (2 * expected_zoom), .05,
         'Projection at actual zoom')
    near(view.get('screenPixelsPerTexturePixel'), (top - bottom) / actor['actualFrameHeight'], .0001, 'Actual texture magnification')
    feet = view.get('actorFeetScreenPixels', {})
    require(all(finite_number(feet.get(key)) for key in ('x', 'y', 'z')) and
            0 <= feet['x'] <= 1920 and 0 <= feet['y'] <= 1080 and feet['z'] > 0, 'Actual actor feet are outside the screenshot')
    inside = left >= 0 and bottom >= 0 and right <= 1920 and top <= 1080
    require(type(view.get('fullFrameInsideCapture')) is bool and view['fullFrameInsideCapture'] == inside,
...
def check_runtime_visual_review(path, report_path, snapshot_path, report, views):
    require(path is not None and Path(path).is_file(), 'Publication needs a saved review of the actual normal/nearest runtime screenshots')
    review = read(path)
    require(review.get('schema') == 'qdao-original-v14-hd/runtime-visual-review-v1' and review.get('status') == 'passed' and
            isinstance(review.get('reviewer'), str) and review['reviewer'].strip(), 'Runtime visual review is incomplete')
    require(review.get('runtime_report_sha256') == sha(report_path) and review.get('input_snapshot_sha256') == sha(snapshot_path),
            'Runtime visual review refers to another report/input')
    require(utc(review['reviewed_utc']) >= utc(report['generatedUtc']), 'Runtime screenshot review predates actual observation')
    records = review.get('views', [])
    require(isinstance(records, list) and all(isinstance(record, dict) for record in records), 'Runtime visual review views are invalid')
    keys = [(record.get('character_id'), record.get('view')) for record in records]
    require(len(keys) == len(set(keys)), 'Runtime screenshot review contains duplicate views')
    lookup = dict(zip(keys, records))
    for key, observed in views.items():
        record = lookup.get(key, {})
        require(record.get('status') == 'passed' and record.get('image_sha256') == observed['imageSha256'] and
                isinstance(record.get('notes'), str) and record['notes'].strip() and record.get('clipping_reviewed') is True and
                type(record.get('full_frame_inside_capture')) is bool and record['full_frame_inside_capture'] == observed['fullFrameInsideCapture'],
                'Actual runtime view lacks an exact visual/clipping review: ' + str(key))
    return sha(path)
...
def runtime_acceptance(plans, project, arguments):
    ...
    check_results(arguments.playmode_results, 'QdaoRosterSandboxPlayModeTests.RealCitySandbox_SwitchesAllAvailableAppearancesWithoutReplacingThePlayer_AndWalksWithTheRealMotor', hd_platform='PlayMode')
    ...
    check_launch_binding(arguments.editmode_results, report, rows, plans, platform='EditMode')
    play_launch = check_launch_binding(arguments.playmode_results, report, rows, plans, sha(arguments.input_snapshot), platform='PlayMode')
    capture_root = Path(play_launch['capture_directory']).resolve()
    require(capture_root == arguments.runtime_report.parent.resolve(), 'Runtime captures belong to another launch directory')
    camera = camera_contract(project)
    require(camera['minimum'] < camera['default'], 'Nearest view must be closer than normal view')
    reviewed_views = {}
    ...
    for plan in plans:
        ...
        check_runtime_inventory(rows, plan, require_index=True)
        for view_name in ('normalView', 'nearestView'):
            reviewed_views[(character, view_name)] = check_runtime_view(actor, view_name, capture_root, camera)
        require(actor.get('actualIdleMatchResources') is True and
                actor.get('actualDedicatedIdleDirections') == {direction: 1 for direction in DIRECTIONS},
                'All eight dedicated idle textures must match actual Resources')
    ...
    visual_review_sha = check_runtime_visual_review(getattr(arguments, 'runtime_visual_review', None),
        arguments.runtime_report, arguments.input_snapshot, report, reviewed_views)
    unity_evidence = {}
    for platform, xml_path in (('EditMode', arguments.editmode_results), ('PlayMode', arguments.playmode_results)):
        xml_path = Path(xml_path)
        unity_evidence[platform] = {label: sha(path) for label, path in {
            'xmlSha256': xml_path, 'launchSha256': xml_path.with_name(xml_path.stem + '-launch.json'),
            'completionSha256': xml_path.with_name(xml_path.stem + '-completion.json'),
            'logSha256': xml_path.with_suffix('.log')}.items()}
    return {'runtimeVisualReviewSha256': visual_review_sha, 'unityEvidence': unity_evidence,
            'runtimeViews': [{'characterId': key[0], 'view': key[1], **value} for key, value in reviewed_views.items()],
            'runtimeReportSha256': sha(arguments.runtime_report), 'inputSnapshotSha256': sha(arguments.input_snapshot),
...
def target_state(project, plan):
    ...
    prefix = FAMILY + '/' + plan['characterId'] + '/'
    rows = {prefix + path.relative_to(target).as_posix(): {'sha256': sha(safe_child(target, path.relative_to(target)))}
            for path in target.rglob('*') if path.is_file()}
    check_runtime_inventory(rows, plan)
...
def main(argv=None):
    ...
    parser.add_argument('--runtime-visual-review', type=Path, help='Passed review bound to actual runtime report/input and normal/nearest PNG SHA/clipping; required only for publish')
    ...
    audit = {'schema': 'qdao-original-roster-v14-hd/publication-v1', 'publicationToolSha256': sha(Path(__file__)), 'createdUtc': datetime.now(timezone.utc).isoformat(),
