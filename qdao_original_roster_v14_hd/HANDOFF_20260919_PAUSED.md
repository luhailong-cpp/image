# 原版23名角色动画｜暂停交接（2026-09-19）

**当前状态：用户要求暂停，剩余任务以后继续。全部任务未完成，正式接入仍为4/23（00–03）。**

最新用户原话：“这样吧 剩下的东西你写个交接文档，我们以后再继续做”。本文件是下一窗口第一入口，优先于2026-09-18旧交接中的暂停型号、数量和工具状态。停止新增绘图；已发出的调用只归档，已启动处理收尾。未创建定时任务或后台续画。

## 1. 用户目标和仍有效的授权

原目标：提交 `9adcf9291e4a867601868889a5965f3cd48630ba` 中原版00–22共23名；每名8方向×16真实行走帧，另8独立idle；30ms/帧、480ms/周期，原移动速度9、原肖像身份不改，最后真实Unity验证并接入正式客户端。
...
- 身份权威清单：`../qdao_original_roster_v13/inventory.json`，用character_id字段，不能误用带transparent_4096后缀的source_id；旧character_move_8dir是00的别名，不是第24名。
- 本次逐角色、逐方向缺槽：[暂停库存JSON](CONTINUATION_STATE_20260919_PAUSED.json)。数字是文件库存，不是视觉合格数；不把孤立raw、strip、pilot或拒稿算完成。
...
| 07–22（16名） | 0 | 0 | 0 | 各128，另各缺8idle |

按上述已导入候选，仍缺 **2229walk+128idle=2357个动作槽**。有待修或未完整审核的候选可能还需返工。这不是“再画2357张就一定验收”的承诺。
...
7. 再完善05/06顺序发布支持，并继续07–22。全部23名正式接入且真实验收完成才可说“全部做完”。
