"""Check accepted Original 00-22 candidates; stage/publish only with explicit --execute.
...
RESOURCE_FAMILY = 'World/Characters/QdaoOriginalRosterV13'
...
} | {f'walk/{d}/strip.png' for d in DIRECTIONS}
...
def approved_candidate(character, entry):
    ...
    require(manifest.get('character_id') == character and manifest.get('version') == 13 and
    ...
    require(alignment.get('alignment_version') == 2 and alignment.get('root_px') == [256, 471] and
    ...
    require(len(files) == len(manifest['files']) == 145 and set(files) == EXPECTED_PNGS,
            'Candidate must contain precisely 128 walk, eight idle, eight strips and one portrait')
    ...
    for key in ('normal_size_review', 'enlarged_review', 'seam_15_16_01_review', 'anatomical_contacts_01_09_review'):
    ...
    require(saved.get('status') == 'passed' and saved.get('visual_review') == 'passed' and
            saved.get('scope') == 'all8' and saved.get('character_id') == character and saved.get('version') == 13 and
    ...
    activation = {'version': 13, 'characterId': character, 'frameCount': 16, 'frameDurationMs': 30,
...
def runtime_acceptance(plans, project, arguments):
    ...
    for plan in plans:
        ...
        require(actor.get('resourceFolder') == RESOURCE_FAMILY + '/' + character and
                actor.get('actualArtworkVersion') == actor.get('catalogVersion') == 13 and actor.get('actualFrameCount') == 16 and
    ...
    return {'runtimeReportSha256': sha(arguments.runtime_report), 'inputSnapshotSha256': sha(arguments.input_snapshot),
            ...
            'testedOriginalCount': report['testedOriginalCount'], 'baselineReportSha256': sha(arguments.baseline_report)}
...
def target_state(project, plan):
    ...
    files = {path.relative_to(target).as_posix(): sha(safe_child(target, path.relative_to(target)))
             for path in target.rglob('*') if path.is_file() and not path.name.endswith('.meta')}
...
def execute(plans, project, audit):
    ...
    for plan in plans:
        ...
        if state == 'already_identical':
            continue
        staging = safe_child(project, 'Temp/QdaoOriginalRosterV13-' + uuid.uuid4().hex + '/' + plan['characterId'])
...
def main(argv=None):
    ...
    parser.add_argument('--baseline-report', type=Path, default=ROOT / 'runtime-validation/contract-run1/city-captures/runtime-observed-appearances.json')
    parser.add_argument('--baseline-snapshot', type=Path, default=ROOT / 'runtime-validation/input-snapshot-run1-playmode.json')
    arguments = parser.parse_args(argv)
    audit = {'schema': 'qdao-original-roster-v13/publication-v1', 'createdUtc': datetime.now(timezone.utc).isoformat(),
    ...
    try:
        ...
        for character in selected:
            try:
                ...
                audit['candidates'].append({'characterId': character, 'pngCount': 145, 'resourceFileCount': len(plan['outputs']),
