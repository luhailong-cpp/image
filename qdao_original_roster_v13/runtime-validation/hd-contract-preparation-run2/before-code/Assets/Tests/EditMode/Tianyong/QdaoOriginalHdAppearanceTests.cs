    ...
    public sealed class QdaoOriginalHdAppearanceTests
    {
        ...
        public void DerivedIndexRejectsStaleRevisionDuplicateMissingAndChangedDimensionsWithoutTextureLoads()
        {
            ...
            try
            {
                ...
                index.entries = QdaoOriginalHdResourceIndex.RequiredRelativePaths().Select(path => new QdaoOriginalHdResourceIndex.Entry
                { path = path, width = 1024, height = 1024, assetGuid = new string('b', 32), sha256 = Sha, sourceBytes = 100 }).ToArray();
                var probes = 0;
