...
const out='E:/work/image/qdao_original_roster_v13/review/01_ice_sword_girl/browser-final-candidate';
fs.mkdirSync(out,{recursive:true});
const results={scope:'01 actual candidate preview, not Unity or final visual approval',started_utc:new Date().toISOString(),base:'http://127.0.0.1:8874/',errors:[],network:[],directions:[],checks:[]};
...
page.on('pageerror',e=>results.errors.push(String(e)));page.on('response',r=>{if(r.url().includes('/candidate/01_ice_sword_girl/'))results.network.push({url:r.url(),status:r.status()});});
await page.goto(results.base,{waitUntil:'networkidle'});
await page.selectOption('#character','01_ice_sword_girl');
