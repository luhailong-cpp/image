...
def main():
 ...
 require(len(files)==145 and not qc['errors'],'Complete, numerically valid character required')
 ...
 require(review.get('reviewed_artifacts')==files,'Visual review must bind the exact145 PNGs')
 ...
 for name in ('normal_size_review','enlarged_review','seam_15_16_01_review','anatomical_contacts_01_09_review'):require(review.get(name) is True,f'Missing actual inspection: {name}')
 ...
 if not a.execute:print(json.dumps({'status':'ready_dry_run','character':a.character,'pngs':145,'writesPerformed':False}));return
