#!/usr/bin/env python3
"""Mechanical Q64 Tianyong r09_c08 4x4 native-patch assembly. No generation or source resizing."""
...
HELPERS = Path(r"E:/work/image/tianyong_festival_hd_20260910/seam_helpers.py")
GRID = 4
CORE = 1024
HALO = 115
OVERLAP = 2 * HALO
...
FINAL = GRID * CORE
OUTPUT = ROOT / "output_resume_20260918"
QA = ROOT / "qa_resume_20260918"
ART = OUTPUT / "tianyong_r09_c08_q64_4k_candidate.png"
...
def load_sources() -> tuple[list[list[np.ndarray]], list[dict]]:
    ...
    for row in range(1, GRID + 1):
        ...
        for column in range(1, GRID + 1):
            ...
            if len(references) not in (1,4) or record["toolCall"]["num_last_images_to_include"] != len(references):
                raise ValueError(f"Incorrect multi-reference record: {tile_id}")
            if record["selectedTargetImageOneBased"] != (column if len(references)==4 else 1):
                raise ValueError(f"Incorrect selected target image: {tile_id}")
            for reference in references:
                if sha256(Path(reference["path"])) != reference["sha256"]:
                    raise ValueError(f"Submitted reference changed: {reference['path']}")
...
def write_qa(image: Image.Image) -> list[dict]:
    ...
    crops = {
        "stone_upper_left_100pct": (350, 300, 1250, 1200),
        "carving_lower_left_100pct": (500, 2800, 1400, 3700),
        "planter_stairs_lower_right_100pct": (2800, 2800, 3700, 3700),
    }
...
def assemble() -> dict:
    ...
    if not (plan.get("guidePreparation", {}).get("allSharedOverlapPixelsIdentical") is True and plan["guidePreparation"]["sharedOverlapChecks"]==24):
    ...
    manifest = {
        ...
        "published": False,
        "scope": "One more-Q/chibi Tianyong festival r09_c08 4096-square candidate at 65536-square whole-city density, covering 18.75x18.75 world units, from 16 native1254 patches; not a complete city map.",
        "backendModelVerified": False,
        "backendModel": None,
        ...
        "qa": qa_items,
        "visualQa": {"status": "pending", "note": "Numeric seam differences are diagnostic only; Q-style consistency, paving/carving/planter geometry, stairs and all seam intersections require visual review."},
